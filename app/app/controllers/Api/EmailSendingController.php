...
class EmailSendingController extends ApiBaseController
{
    /**
     ...
     * @var UserRepository
     */
    private $emailSendingRepository;
    ...
    public function __construct(Request $request, Response $response)
    {
        ...
        $this->emailSendingRepository = $this->manager->getRepository(EmailSending::class);
    ...
    {
        try {
            $emailsSendings = $this->emailSendingRepository
                ->pagination($this->request->get('currentPage'), $this->request->get('perPage'))
                ->getResult();

            $total = $this->emailSendingRepository
                ->findTotal()
                ->getTotal();
        ...
        $data = [];
        foreach ($emailsSendings as $item) {
            $data[] = [
                'id' => $item['id'],
                'name' => $item['name'],
                'text' => $item['text'],
